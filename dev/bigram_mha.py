...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.sa_heads = MultiHeadAttention(n_heads=4, head_size=n_embd//4) # 4 heads of 8-dimensional self-attention
        self.lm_head = nn.Linear(n_embd, vocab_size)
    
    def forward(self, idx, targets=None):
        # idx and targets are both of shape (B, T)
        B, T = idx.shape
        tok_emb = self.token_embdding_table(idx) # (B, T, C=n_embd)
        pos_emb = self.position_embdding_table(torch.arange(T, device=device)) # (T, C=n_embd)
        x = tok_emb + pos_emb # (B, T, C=n_embd)
        x = self.sa_heads(x) # apply the single-head self-attention
