...
eval_interval = 500
learning_rate = 3e-4 # self-attention model is very sensitive to the learning rate
...
class GPTLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is None:
            ...
            loss = F.cross_entropy(logits, targets) # (B*T, C), (B*T) --> (1,)
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            idx_cond = idx[:, -block_size:] # 从后往前取block_size个
            # get predictions
            logits, loss = self(idx_cond) # calling forward to get the logits
            # focus only the last predictions
